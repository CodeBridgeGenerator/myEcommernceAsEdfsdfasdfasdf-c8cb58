...
const createUserProfileInDB = async (app, superAdmin, data) => {
  if (superAdmin === data.email) {
    ...
    const userAdminRole = await app.service("roles").find({ query: { name: "Super" } });
    const userAdminPosition = await app.service("positions").find({ query: { name: "Admin" } });

    await app.service("profiles").create({
      name: `${data.name} - Admin`, // Profile name: username + position name
      userId: data._id,
    ...
    if (userInvite.data.length > 0) {
      ...
      if (positions.length > 0) {
        for (const positionId of positions) {
          ...
          let roleId = roles[positions.indexOf(positionId)]; // Match role with position index
          if (!roleId) {
            const defaultRole = await app.service("roles").find({ query: { isDefault: true } });
        ...
        const defaultRole = await app.service("roles").find({ query: { isDefault: true } });
        const defaultPosition = await app.service("positions").find({ query: { isDefault: true } });

        await app.service("profiles").create({
          name: `${data.name} - ${defaultPosition.data[0].name}`, // username + position name
          userId: data._id,
...
// const getDefaultRole = async (app) => {
//   const defaultRole = await app.service("roles").find({
//     query: {
//       isDefault: true,
//     },
//   });
//   return defaultRole.data[0];
// };

// Helper function to get the default position
// const getDefaultPosition = async (app) => {
//   const defaultPosition = await app.service("positions").find({
//     query: {
//       isDefault: true,
//     },
//   });
//   return defaultPosition.data[0];
// };
        ...
        job.data.forEach((data) => {
          const _mail = {
            ...
            data: {
              name: data.name,
              projectLabel:
                process.env.PROJECT_LABEL ? process.env.PROJECT_LABEL : process.env.PROJECT_NAME,
            },
            status: true,
    ...
    if (job.data) {
      if (Array.isArray(job.data)) {
        ...
        const _mail = {
          ...
          templateId: "onError",
          errorMessage: err.message,
          hook: job.data.hook ? job.data.hook : null,
        };
        app.service("mailQues").create(_mail);
      }
    } else {
